...
class TestSamplesAsync(TestCase):
    def test(self) -> None:
        # Gather samples.
        base_dir = Path(__file__).parents[1]
        samples_dir = base_dir / "samples_async"
        sample_files = []
        for dirpath, _, filenames in os.walk(samples_dir):
            for filename in filenames:
                if filename.endswith(".py") and not filename.startswith("_"):
        ...
        for sample_file in sample_files:
            ...
            with open(sample_file) as f:
                ...
                # Inject async function declaration and indent code.
                source = "async def async_main():\n"
                source += "".join(
                    ["    " + sl for (i, sl) in enumerate(sample_lines) if i > 0]
                )

                # Inject code to run async function.
                source += "import asyncio\n"
                source += "asyncio.run(async_main())\n"
